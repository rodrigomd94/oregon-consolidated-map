...
function createSchoolOfficeholders() {

    map.addSource("school_officeholders", { type: "geojson", data: school_officeholders });                                                 //Add a map source, containing the data from the GeoJSON

    //------------------------ ADDING LAYERS ------------------------//
    map.addLayer({
        id: "school_officeholders_genders",
        type: "fill",
        source: "school_officeholders",
        ...
        paint: {
            "fill-color": {
                property: "gend_ratio",                                                                                   //Defining the fill color based on the gender ratio
    ...
    map.addLayer({
        id: "school_officeholders_parties",
        type: "fill",
        source: "school_officeholders",
        layout: {},
        paint: {
    ...
    map.setLayoutProperty("school_officeholders_genders", "visibility", "none");                                                 //Disable visibility for the non active layers in the initial map load
    map.setLayoutProperty("school_officeholders_parties", "visibility", "none");
    //map.setLayoutProperty("school_officeholders_ages", "visibility", "none");
        ...
        new mapboxgl.Popup()
            ...
            .setHTML(
                `<dl>
                          ...
                          <dt>School Board Members</dt><dd>${e.features[0].properties.officeholders}</dd>
                          </dl>`
            )
            .addTo(map);
    ...
    */
    map.on("click", "school_officeholders_ages", function (e) {
        create_popup(e);
    });
    // Change the cursor to a pointer when the mouse is over the states layer.
    map.on("mouseenter", "school_officeholders_ages", function () {
        map.getCanvas().style.cursor = "pointer";
    });
    // Change it back to a pointer when it leaves.
    map.on("mouseleave", "school_officeholders_ages", function () {
    ...
    */
    map.on("click", "school_officeholders_genders", function (e) {
        create_popup(e);
    });
    // Change the cursor to a pointer when the mouse is over the states layer.
    map.on("mouseenter", "school_officeholders_genders", function () {
        map.getCanvas().style.cursor = "pointer";
    });
    // Change it back to a pointer when it leaves.
    map.on("mouseleave", "school_officeholders_genders", function () {
        map.getCanvas().style.cursor = "";
    });
    /* 
    ------POPUP for the parties layer ---------------
    */
    map.on("click", "school_officeholders_parties", function (e) {
        create_popup(e);

    });
    // Change the cursor to a pointer when the mouse is over the states layer.
    map.on("mouseenter", "school_officeholders_parties", function () {
        map.getCanvas().style.cursor = "pointer";
    });
    // Change it back to a pointer when it leaves.
    map.on("mouseleave", "school_officeholders_parties", function () {
        map.getCanvas().style.cursor = "";
    });
...
---------------------------------ages legend ------------------*/
    ...
    for (i = 0; i < intervals_age.length; i++) {
        ...
        school_officeholders_ages_legend.appendChild(item);                                                   //Appending the container to the legend container
    ...
    for (i = 0; i < intervals_female.length; i++) {
        ...
        school_officeholders_female_legend.appendChild(item);
    }
    //.........male.................................
    ...
    for (i = 0; i < intervals_male.length; i++) {
        ...
        item.appendChild(key);
        item.appendChild(value);
        school_officeholders_male_legend.appendChild(item);
    ...
    for (i = 0; i < intervals_republican.length; i++) {
        ...
        item.appendChild(key);
        item.appendChild(value);
        school_officeholders_republican_legend.appendChild(item);
    }
    //.............Democrats ........................
    ...
    for (i = 0; i < intervals_democrat.length; i++) {
        ...
        item.appendChild(key);
        item.appendChild(value);
        school_officeholders_democrat_legend.appendChild(item);
    }
    //.............Nonaffiliated ....................
    ...
    for (i = 0; i < intervals_nonaffiliated.length; i++) {
        ...
        item.appendChild(key);
        item.appendChild(value);
        school_officeholders_nonaffiliated_legend.appendChild(item);
    }
    //............-No dominance .....................
    ...
    for (i = 0; i < intervals_nodominant.length; i++) {
        ...
        item.appendChild(key);
        item.appendChild(value);
        school_officeholders_nodominant_legend.appendChild(item);
    }
